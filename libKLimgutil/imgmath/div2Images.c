/* Function: div2Images */
/* Version: 1.0.1 
 * 	Divide one image or section by another.  A shift can be applied to 
 ...
 *
 * Calling div2Images
 *	status = div2Images(ppix1, ppix2, presult, naxes, section, dx, dy);
 *
 * Needs :
 */
	...
	long naxes[], unsigned long section[], long dx, long dy)
{
 ...
 for (jj=section[2]-1,ll=0; jj<section[3]; jj++,ll++) {
   for (ii=section[0]-1,kk=0; ii<section[1]; ii++,kk++) {
     if (*(*(ppix2+jj-dy)+ii-dx) == 0) {		/*division by zero; one pixel*/
       *(*(presult+ll)+kk) = BIGNUMBER;
     }
     else {
