/* Function: divImage */
/* Version: 1.0.1 
 * 	Divide one image or section by some value.
 *
 * int divImage(double **ppix1, double value, double **presult,
 ...
 *
 * Calling divImage
 *	status = divImage(ppix1, value, presult, naxes, section);
 *
 * Needs :
 */
	...
	long naxes[], unsigned long section[])
{
 ...
 if (value == 0) return(DIVISION_BY_ZERO);
